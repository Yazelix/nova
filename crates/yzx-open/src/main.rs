...
fn try_bridge(config: &Config, targets: &[PathBuf], working_dir: &Path) -> Result<bool> {
    ...
    for (path, registry) in candidates {
        ...
        let (action, payload) = bridge_open_request(targets, working_dir);
        log_debug(
            config,
            &format!(
...
"#,
    ...
    #[test]
    fn retarget_requires_one_target() {

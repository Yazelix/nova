...
- New tabs start in Yazi's fast zoxide-backed recent-folder search. `Tab` and
  ...
  folder without changing the tab folder or opening the editor. Search
  ...
  folder when a file or nothing is hovered. The startup browser's ordinary open
  action creates its first editor pane, while popup opens preserve the existing
  tab folder. `Shift Tab` spots the hovered file, empty history leaves Yazi ready
  to browse, and the former global `Alt z` action remains absent. Both managed
  views use clean, pill-free key footers, and the command menu provides a
  discoverable route to the persistent popup.
...
- Right-side widgets disappear from left to right as tabs need room. Surviving
  widgets remain anchored, and the corner version badge yields last.
...
- The top bar preserves tabs by removing complete widgets from the right as
  ...
  its name shortens only after neighboring tabs are removed. Hidden widgets keep
  updating so they restore current content when the window widens.
...
- Packaged Nushell, Bash, Zsh, and Fish use Atuin 18.16.1 for local history and
  ...
  deleting data. User Atuin remains the sole hook owner; Nova honors
  `ATUIN_NOBIND`, disables Atuin AI bindings, and keeps Carapace as the Nushell
...
- `Alt r` and `yzx reveal <target>` show and focus the active tab's persistent
  ...
  Zellij's actual floating-layer visibility. In the Yazi popup, `Alt r` hides
  the popup and returns to the underlying pane without opening the hovered
  item; tiled Yazi focuses the unchanged editor and remains visible. `Enter`
  remains the explicit file-open action. Helix and Yazi own their local `Alt r`
  actions directly; Zellij no longer reroutes the key through a cached focus
  snapshot, so hiding the popup cannot replay the chord into Helix and reopen
  it.
- Every managed `keybindings.*` action accepts `false` to leave its shortcut
  ...
  and popup behavior remain unchanged. Ratconfig labels the explicit state
  `Unmapped`, while reset restores the packaged default on the next launch.
...
- `yzx enter` is the supported headless and SSH route for the managed
...
- First use seeds child-owned `cursors.toml` without overwriting user state.
  ...
  file. `yzx launch` passes that exact path to Mars. Mars currently consumes
...
- Root `config.toml` is a sparse explicit-override document. Startup and
  `yzx config` inherit packaged values without creating or completing the user
  ...
  metadata.
- Opening `yzx config` no longer creates `mars/config.toml`. The Mars tab shows
...
- User-visible product surfaces identify the runtime as Yazelix Nova and use
  the canonical `yzx` command and Yazelix config roots.
- `yzx config` fully redraws after returning from an external editor instead of
...
- `yzx config` has a dedicated `popups` tab for popup margins and managed
  config/agent/Git/menu popup keybindings. Root `config.toml` remains the source
  for these fields.
- `yzx config` Zellij tab saves and resets update the active managed session
...
- The default `yzx` package no longer pulls Mars' removed SerenityOS emoji
...
- `yzx config` now declares `mars.appearance.preset` as the Ratconfig UI theme
  ...
  already open are still picked up on the next `yzx config` launch.
- `yzx config` native file rows now show absent optional files as neutral
  `absent` rows instead of warning-colored `missing` rows.
- The root `[appearance].mode` setting was removed before release. Mars
  appearance stays Mars-owned: `yzx config` exposes the native
...
- Flake package and app outputs are exposed for `x86_64-darwin` and
  ...
  package-first: `yzx help`, `status`, `doctor`, and `enter` are the supported
  floor, while Mars-backed `launch` remains issue-driven best-effort until
  trusted macOS hardware validation confirms it.
- `yzx tutor` prints guided Yazelix lessons for workspace flow, discovery,
  troubleshooting, and native tool tutors. `yzx tutor hx`/`helix` and
  `yzx tutor nu`/`nushell` print the packaged Helix and Nushell tutor commands
...
- `yzx help` prints help, `yzx env` opens the configured managed shell without
  launching the UI with packaged `hx`, `lazygit`, and `git` on PATH, `yzx enter`
  starts the managed Zellij runtime in the current terminal, and `yzx launch`
...
- Root `config.toml` supports `[editor].command`, defaulting to `yzx-hx`.
  ...
  stay user-owned. `yzx status` and `yzx doctor` report the configured and
...
- The `yzx config` Helix tab opens managed `helix/config.toml`,
  `helix/languages.toml`, `helix/helix.scm`, and `helix/init.scm` in `yzx-hx`.
  Files are created only when activated; either Steel row creates the pair. A
  new `helix/config.toml` is a comment-only sparse override. `yzx-hx` generates
  ...
  user-overridable. `yzx doctor` warns when an override sets reserved `Alt r`.
- Managed Helix loads the packaged `:yzx-new-shell` Steel command by default.
...
- `yzx menu` opens a packaged `fzf` live-filter command palette and uses the
  same command descriptions as `yzx help` for its curated command list.
- `yzx` uses a Rust front door for startup setup and final process handoff:
...
- `yzx screen [style]` shows a Yazelix terminal screen directly. Startup runs a
...
- `yzx config` ignores unsupported modified terminal keys instead of treating
  them as text.
- `yzx config` restores raw terminal mode if alternate-screen setup fails.
- Yazi opens reuse only a Helix bridge pane in the invoking Zellij tab. `Alt z`
  moves to the zoxide-selected directory, sends it through `yzx-open`, renames
...
- `Alt r` reveals the current Helix buffer in the managed Yazi sidebar, and
  `yzx reveal <target>` exposes the same active-tab reveal path inside a
...
- `config.toml` controls `open.log_level`, `shell.program`,
  ...
  markers for stable pane identity, and `yzx config` shows scalar root fields in the main config
...
- `yzx` appends `~/.config/yazelix/zellij/config.kdl` as a native Zellij
...
- `yzx` reads `~/.config/yazelix/zellij/plugins.kdl` as a plugin-only
...
- `Alt Shift J/K/L/M` toggle Git, config, agent, and menu popups through
  ...
  killing the agent process, `yzx menu` opens a live-filter command palette for
...
- `yzx` installs a Nix/Lix-compatible flake runtime that opens Mars with the
  Yazelix Zellij fork.
...
- Nushell loads packaged Starship, Carapace, and Zoxide setup before optional
  user files under `~/.config/yazelix/nu`.
- Yazi opens files and directories through `yzx-open`, reusing a live Yazelix
  Helix bridge inside the current `yzx` window when possible.
- Yazi `Alt z` opens a zoxide picker and sends the selected directory through
  `yzx-open`.
- `yzx-open` writes bounded rotated diagnostics and honors `YZX_OPEN_LOG`.
- Profile installs include a `Yazelix Nova` desktop entry.
